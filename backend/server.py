...
The knowledge content (problems, lessons) is static JSON generated by
build_content.py and fetched straight from the app; this API owns just the
things a user changes: saved code, test runs and timed attempts. Search, facets,
lesson progress and the stats page are folded in the browser from
GET /api/progress plus that content.

No auth (single implicit user). Problems are identified by their path-based id
(e.g. "maximum-subarray" or "CTCI/1.1-is-unique"). Ids can contain "/", so
id-specific endpoints take the id as a query parameter or request-body field
...
def _attempt_fields(a) -> dict:
  ...
  `a` is either the latest attempt row or None (no attempt, i.e. "not-started").
  This is the single source of truth for the derived status; the client's
  progress.ts mirrors the "not-started" branch.
  """
