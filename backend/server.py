...
user.current_user() (a cookie today, a token claim once auth exists), but the
SQLite storage below is still single-user: the id is validated and threaded to
the handlers, not yet written to the tables.
...
# way in. `user` is deliberately unused in the bodies: storage is still
# single-user (see db.py), and the next ticket makes the handlers pass it to the
# db layer without touching a single signature here.
